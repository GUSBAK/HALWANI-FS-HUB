...
let visitActions = [];
let pickedProducts = [];
let activePlanMonth = currentMonth();
...
function allCustomers(){
  const map = new Map();
  [...(DATA.customers||[]), ...manualCustomers].forEach(c=>{ if(c && customerKey(c)) map.set(customerKey(c), c); });
...
function reports(){
  ...
  ${isManager()?`<section class="card"><h3>Admin Tools</h3><p class="muted">Load the monthly journey plan and account collection targets for pilot testing.</p><div class="actions"><button class="primary" onclick="route('importHub')">Import Monthly Data</button><button onclick="route('lists')">Master Lists</button></div></section>`:''}`);
}
function importHub(){
  setTitle('Import Monthly Data');
  screen(`<section class="card"><h3>Import Monthly Data</h3><p class="admin-note">Static pilot only: imported files are saved in this browser. The final cloud-backend phase will use the same columns and make the data visible to every salesperson.</p>
    <label>Journey Plan CSV</label><input class="file-input" id="journeyImport" type="file" accept=".csv,text/csv">
    <button style="width:100%;margin-top:10px" onclick="downloadJourneyTemplate()">Download Journey Plan Template</button>
    <div class="soft-divider"></div>
    <label>Collection Targets CSV</label><input class="file-input" id="collectionImport" type="file" accept=".csv,text/csv">
    <button style="width:100%;margin-top:10px" onclick="downloadCollectionTemplate()">Download Collections Template</button>
    <div class="soft-divider"></div>
    <button class="primary submit-big" onclick="importMonthlyData()">Import Selected Files</button>
    <button class="danger" style="width:100%;margin-top:10px" onclick="clearImportedData()">Clear Imported Pilot Data</button>
  </section>`);
}
function parseCSV(text){
  ...
  const headers=rows.shift().map(h=>normalize(h).replace(/[^a-z0-9]/g,''));
  return rows.map(r=>Object.fromEntries(headers.map((h,i)=>[h,String(r[i]||'').trim()])));
}
function mapJourneyRow(r){ return {month:r.month||String(r.date||'').slice(0,7),salesman:r.salesman||'',date:r.date||'',time:r.time||'',customerCode:r.customercode||'',customerName:r.customername||'',branch:r.branch||'',city:r.city||'',area:r.area||'',collectionTargetSAR:r.collectiontargetsar||r.collectiontarget||'',visitObjective:r.visitobjective||'',notes:r.notes||''}; }
function mapCollectionRow(r){ return {month:r.month||String(r.duedate||'').slice(0,7),salesman:r.salesman||'',customerCode:r.customercode||'',customerName:r.customername||'',collectionTargetSAR:r.collectiontargetsar||r.collectiontarget||'',dueDate:r.duedate||'',notes:r.notes||''}; }
function readImportFile(input){ return new Promise((resolve,reject)=>{const f=input?.files?.[0];if(!f){resolve(null);return;}const reader=new FileReader();reader.onload=e=>resolve(parseCSV(String(e.target.result||'')));reader.onerror=reject;reader.readAsText(f);}); }
async function importMonthlyData(){
  try{
    const journeyRows=await readImportFile($('#journeyImport')); const collectionRows=await readImportFile($('#collectionImport'));
    if(!journeyRows&&!collectionRows){toast('Choose at least one CSV file');return;}
    if(journeyRows){journeyPlan=journeyRows.map(mapJourneyRow).filter(r=>r.date&&r.customerCode);}
    if(collectionRows){collectionTargets=collectionRows.map(mapCollectionRow).filter(r=>r.customerCode&&r.collectionTargetSAR!=='');}
    saveAll();activePlanMonth=currentMonth();toast(`Imported ${journeyRows?journeyPlan.length:0} plan rows and ${collectionRows?collectionTargets.length:0} collection targets`);route('plan');
  }catch(e){toast('Could not read the CSV file. Check the template and try again.');}
}
function clearImportedData(){ if(!confirm('Clear imported journey plan and collection targets from this browser?'))return; journeyPlan=[];collectionTargets=[];saveAll();toast('Imported pilot data cleared');route('reports'); }
...
function downloadJourneyTemplate(){ downloadFile('monthly_journey_plan_template.csv','month,salesman,date,time,customerCode,customerName,branch,city,area,collectionTargetSAR,visitObjective,notes\n2026-07,Ahmed Nabil,2026-07-01,08:30,142407,شركة المأكولات السريعة,Jeddah,Jeddah,Prince Sultan,25000,Collection,Collect June overdue amount\n'); }
function downloadCollectionTemplate(){ downloadFile('monthly_collection_targets_template.csv','month,salesman,customerCode,customerName,collectionTargetSAR,dueDate,notes\n2026-07,Ahmed Nabil,142407,شركة المأكولات السريعة,25000,2026-07-01,Overdue June balance\n'); }
